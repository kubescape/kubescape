...
func TestDecryptContainerMetadataUnstructured(t *testing.T) {
					...
					"containers": []any{
						map[string]any{
							...
							"env": []any{
								...
								map[string]any{"name": enc("SECRET_REF"), "valueFrom": map[string]any{
	...
	assert.Equal(t, "SECRET_REF", env[1].(map[string]any)["name"])
...
func TestDecryptContainerMetadataTyped(t *testing.T) {
	...
	podSpec := map[string]any{
		"containers": []corev1.Container{{
			...
			Env: []corev1.EnvVar{
				...
				{Name: enc("SECRET"), ValueFrom: &corev1.EnvVarSource{SecretKeyRef: &corev1.SecretKeySelector{LocalObjectReference: corev1.LocalObjectReference{Name: enc("typed-secret")}}}},
	...
	assert.Equal(t, "SECRET", containers[0].Env[1].Name)
