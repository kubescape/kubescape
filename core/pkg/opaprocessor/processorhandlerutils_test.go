...
func TestMatchesRuleObjects(t *testing.T) {
	...
	assert.True(t, matchesRuleObjects("core", "v1", "pods", "Pod", matchers), "Pod with core group should match")
...
func TestWorkloadMatchesRuleObjects(t *testing.T) {
	matchers := []reporthandling.RuleMatchObjects{
		{
			APIGroups:   []string{""},
			APIVersions: []string{"v1"},
	...
	assert.True(t, workloadMatchesRuleObjects(pod, matchers))
	...
	assert.False(t, workloadMatchesRuleObjects(cm, matchers))
	assert.False(t, workloadMatchesRuleObjects(nil, matchers))
}
