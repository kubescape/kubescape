...
// TestResourceBasePath_RejectsSourcePathOutsideScannedTree pins the traversal defence on
// the report-supplied Source.Path: it may narrow the scanned root but never leave it,
// because the result is joined into a path this package writes to.
func TestResourceBasePath_RejectsSourcePathOutsideScannedTree(t *testing.T) {
	base, err := filepath.EvalSymlinks(t.TempDir())
	require.NoError(t, err)
	require.NoError(t, os.MkdirAll(filepath.Join(base, "manifests"), 0o750))

	outside, err := filepath.EvalSymlinks(t.TempDir())
	require.NoError(t, err)

	// a symlink inside the scanned tree pointing out of it must not launder the escape
	linkPath := filepath.Join(base, "escape-link")
	...
	tests := []struct {
		...
		{name: "no source falls back to the report-wide root", sourcePath: "", want: base},
		{name: "the scanned root itself is accepted", sourcePath: base, want: base},
		{name: "a subdirectory of the scanned root is accepted", sourcePath: filepath.Join(base, "manifests"), want: filepath.Join(base, "manifests")},
		{name: "a parent traversal is rejected", sourcePath: filepath.Join(base, "..", ".."), want: base},
		{name: "an unrelated absolute path is rejected", sourcePath: outside, want: base},
		{name: "a symlink out of the tree is rejected", sourcePath: linkPath, want: base},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := &FixHandler{localBasePath: base}

			resource := &reporthandling.Resource{}
			if tt.sourcePath != "" {
				resource.Source = &reporthandling.Source{Path: tt.sourcePath}
			}
