...
func TestNewFixHandler_EmptyReportGUID(t *testing.T) {
	...
	reportJSON := `{"metadata":{"scanMetadata":{"scanningTarget":2},"contextMetadata":{"directoryContextMetadata":{"basePath":"testdata"}}}}`
	...
	assert.NotEqual(t, "invalid report file: not a valid kubescape scan report. Please provide a JSON file generated by 'kubescape scan --format json'", err.Error())
}
