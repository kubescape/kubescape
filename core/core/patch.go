...
func (ks *Kubescape) Patch(ctx context.Context, patchInfo *ksmetav1.PatchInfo, scanInfo *cautils.ScanInfo) (*models.PresenterConfig, error) {
	...
	if err := copaPatch(ctx, patchInfo.Timeout, patchInfo.BuildkitAddress, patchInfo.Image, fileName, patchedImageName, ""); err != nil {
		return nil, err
	}
...
func copaPatch(ctx context.Context, timeout time.Duration, buildkitAddr, image, reportFile, patchedImageName, workingFolder string) error {
	timeoutCtx, cancel := context.WithTimeout(ctx, timeout)
	defer cancel()

	ch := make(chan error)
	go func() {
		ch <- patchWithContext(timeoutCtx, buildkitAddr, image, reportFile, patchedImageName, workingFolder)
	}()
...
func patchWithContext(ctx context.Context, buildkitAddr, image, reportFile, patchedImageName, workingFolder string) error {
	...
	patchedImageState, _, err := pkgmgr.InstallUpdates(ctx, updates, false)
	if err != nil {
		return err
	}
...
func tryParseScanReport(file string) (*types.UpdateManifest, error) {
	...
	parser := cautils.KubescapeParser{}
