...
func isLegacyEvent(kind, apiVersion string) bool {
	if strings.Contains(apiVersion, "/") {
		parts := strings.SplitN(apiVersion, "/", 2)
		if parts[0] != "" {
			return false
	...
	return NormalizeWorkloadKind(kind) == "Event"
}

// IsLegacyEvent reports whether kind is Event and apiVersion belongs to the core group.
func IsLegacyEvent(kind, apiVersion string) bool {
	return isLegacyEvent(kind, apiVersion)
}
...
func validateWorkloadName(name, kind, apiVersion string) error {
	...
	if isLegacyEvent(kind, apiVersion) || isRBACResource(kind, apiVersion) || isCertificateSigningRequest(kind, apiVersion) || isUnresolvedResource(kind, apiVersion) {
		if errs := content.IsPathSegmentName(name); len(errs) > 0 {
			return fmt.Errorf("%w: invalid workload name %q: %s", ErrInvalidWorkloadIdentifier, name, strings.Join(errs, "; "))
		}
		return nil
