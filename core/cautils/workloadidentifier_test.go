...
func TestParseWorkloadIdentifierString_Invalid(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "kind starting with number",
			input: "1Pod/nginx",
		},
		{
		...
		{
			name:  "Event with events.k8s.io group and colon in name rejected by DNS-1123",
...
func TestParseWorkloadIdentifierString_Values(t *testing.T) {
	testCases := []struct {
		...
		{
			Description:    "valid hyphenated CRD kind with namespace and custom API group",
