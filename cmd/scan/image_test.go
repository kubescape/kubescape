...
func TestGetImageCmd_RunE_FormatFlagInvalid(t *testing.T) {
	...
	err := cmd.RunE(cmd, []string{"nginx"})
	assert.EqualError(t, err, `invalid format "xml", supported formats: pretty-printer, json, junit, prometheus, pdf, html, sarif, gitlab-sast, yaml, cyclonedx-json, spdx-json, otel`)
}
