...
func GetScanCommand(ks meta.IKubescape) *cobra.Command {
	...
	scanCmd.PersistentFlags().Float32VarP(&scanInfo.ComplianceThreshold, "compliance-threshold", "", 0, "Compliance threshold is the percent below which the command fails and returns exit code 1. Applies to 'scan framework', 'scan control', 'scan workload', and '--view resource|control'")
	...
	scanCmd.PersistentFlags().BoolVar(&scanInfo.FailOnStaleDB, "fail-on-stale-db", false, "Fail image scans when the vulnerability database is older than --max-db-age (default: warn only). Can also be set with KS_FAIL_ON_STALE_DB.")
	scanCmd.PersistentFlags().DurationVar(&scanInfo.MaxDBAge, "max-db-age", 0, "Maximum allowed age of the vulnerability database before it is considered stale (e.g. 120h, 7d is 168h). Warns always; fails only with --fail-on-stale-db. 0 selects the default (120h). Can also be set with KS_MAX_DB_AGE.")
